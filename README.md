...
Project import detects Git remote and target branch, GitHub, package manager, Node/TypeScript, Flutter, Rust, Go and Python markers, validation candidates, GitHub Actions workflows/Required Checks, architecture documents, and the official public dependency hosts implied by the detected toolchain. It persists the latter as `operationalConfig.networkPolicy = { mode: "brokered", dependencyHosts: [...] }`. The Human confirms only Intent, delivery direction, and authoritative gates; no `project.json` is required. Private authenticated registries are deliberately unsupported in 1.0.
...
Every supervised Runtime and validation process tree also runs inside an AEC-S-owned, deny-by-default macOS Seatbelt profile. It receives an isolated `HOME`/XDG/temp tree, no SSH agent socket or GitHub token, no user/global Git configuration, no AppleEvent authority, and no macOS Keychain service. Mach service lookup is deny-by-default; `osascript`, Finder/System Events, AppleEvent, TCC, LaunchServices, securityd, and credential helpers are explicitly denied and covered by the functional isolation probe. User-home and per-user temporary data are unreadable except for the exact worktree, read-only controller evidence, AEC-S installation, and the selected Runtime roots. A unique Runtime-output directory is the only writable controller-adjacent tree; JobInput, JobResult, logs, and gate evidence remain controller-owned, and persisted Job input/result records are bound to the same SHA-256 digest. Before a command may pass its start gate, the supervisor durably records its PID and microsecond-resolution macOS `libproc` identity. A 1 ms native descendant ledger follows recorded children across process-group detachment, persists newly observed identities in the supervisor lock, and refuses completion until they converge. Recovery kills the prior command group and recorded detached descendants before another execution may start. Credential, policy, and executable paths such as Codex `auth.json`/`config.toml` and Kimi `config.toml`/`bin` remain explicitly non-writable even when a nonstandard Runtime root is configured. Runtime-owned session, database, cache, index, log, telemetry, and atomic state files are writable because current Codex and Kimi versions create dynamic state names. Kimi's trusted ACP process may also atomically refresh its own credential JSON and OAuth lock; Agent-requested paths remain independently confined to the worktree by one-shot ACP permission checks. DSH credential files remain read-only. Executors receive worktree write access, while Reviewers do not. Credential executables found on the active `PATH`, including `gh`, `ssh`, and Keychain helpers, are denied in addition to standard installation paths. AEC-S functionally probes this kernel-enforced boundary before creating or resuming a Run and marks work operationally blocked if it cannot be enforced; it never falls back to advisory environment filtering or consumes Agent attempts. Apple deprecates the `sandbox-exec` launcher even though it remains shipped, so removal or protocol drift is an explicit fail-closed compatibility event.

Restricted commands, Validation, and Environment Contract probes run with outbound and inbound networking denied. A first-class Runtime can connect only to two ephemeral loopback services selected for its Run: an HTTPS CONNECT proxy for the adapter-owned model-provider allowlist, and an authenticated Run MCP Gateway. The Provider Proxy accepts only exact declared DNS hosts on port 443, resolves and pins a public address, and rejects IP literals, private, loopback, link-local, reserved and cloud-metadata destinations. It does not terminate TLS, inspect bodies, or record URLs, tokens, prompts, or model content. A Runtime upgrade that needs another host therefore fails closed until the adapter declaration is reviewed; user configuration cannot expand this allowlist.

The Run MCP Gateway exposes `aec_s_fetch` to Executors and Reviewers, plus `aec_s_network_exec` to Executors. `fetch` performs unauthenticated public HTTPS GET/HEAD with an 8 MiB limit and rejects authentication headers, URL credentials, private destinations and cross-origin redirects. `network_exec` starts a separate supervised, Seatbelt-confined process with worktree access but no Runtime home, Provider key, GitHub token, SSH agent, user Git credentials or Keychain access; it can reach only the dependency hosts detected at Project import, and rejects publishing, login, token-management, SSH and Git delivery operations. Capability tokens bind the Run, Task Revision, role, Scope and expiry, never enter SQLite, JobInput, events or logs, and are invalid after Gateway closure or daemon recovery. Events retain only host, operation category, result and byte count. Provider services still receive normal development context, and the pinned official Runtime remains part of the trusted computing base; 1.0 does not claim protection against a malicious official Runtime or support authenticated private dependencies.

AEC-S-owned Git commands disable repository hooks and optional filesystem monitors, deny the `ext` transport, and reject repositories whose local configuration declares filters, external diff/text-conversion commands, credential helpers, custom hook paths, or config includes. This prevents repository-controlled Git configuration from becoming an execution path during staging, diffing, commit, worktree creation, or delivery. A Runtime cannot mutate the registered Project's Git metadata, change its `HEAD`, or complete AEC-S commit/push/merge effects; AEC-S rechecks `HEAD` and the final gated diff before publication. An Executor can create arbitrary in-scope files, including files that happen to form a separate local repository, but those files remain ordinary gated worktree content and do not confer Project or remote delivery authority.
