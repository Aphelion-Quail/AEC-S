...
Every supervised Runtime and validation process tree also runs inside an AEC-S-owned macOS Seatbelt profile. It receives an isolated `HOME`/XDG/temp tree, no SSH agent socket or GitHub token, no user/global Git configuration, and no macOS Keychain service. User-home and per-user temporary data are unreadable except for the exact worktree, controller output directory, AEC-S installation, and the selected Runtime roots. Runtime credential/configuration roots are readable but not writable; only enumerated session, cache, index, log, and telemetry paths are writable. Executors receive worktree write access, while Reviewers do not. Credential executables found on the active `PATH`, including `gh`, `ssh`, and Keychain helpers, are denied in addition to standard installation paths. AEC-S functionally probes this kernel-enforced boundary and marks every Runtime unavailable if it cannot be enforced; it never falls back to advisory environment filtering. Apple deprecates the `sandbox-exec` launcher even though it remains shipped, so removal or protocol drift is an explicit fail-closed compatibility event.
